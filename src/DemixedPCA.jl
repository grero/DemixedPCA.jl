...
"""
function compute_dpca(Y::Matrix{Float64}, Yϕ::Matrix{Float64};λ=0.0,kvs...)
  if λ == 0.0
    A_OLS = Yϕ/Y
  else
    A_OLS = ridge(Y, Yϕ, λ)
...
"""
function fit(::Type{dPCA},X::Array{Float64,3},labels::Array{Int64,1};λ=0.0, kvs...)
	ncells,ntrials,nbins = size(X)
